...
def quantize_track(track, ticks_per_quarter, quantization):
    ...
    for i in range(len(cum_msgs)-1):
        msg_temp = cum_msgs[i]
        msg_temp_1 = cum_msgs[i+1]
        if(msg_temp[0] == msg_temp_1[0]):
            if( msg_temp[1].type == 'note_on' and msg_temp_1[1].type == 'note_on'):
                if(msg_temp[1].note == msg_temp_1[1].note and msg_temp[1].velocity == 0 and msg_temp_1[1].velocity >0):
                  msg_temp[1].velocity = msg_temp_1[1].velocity
                  msg_temp_1[1].velocity = 0
                  cum_msgs[i] = msg_temp
                  cum_msgs[i+1] = msg_temp_1
...
def velocity_quantize(mid, quantize_velocity=40):
    quantized_mid = copy.deepcopy(mid)
    # By convention, Track 0 contains metadata and Track 1 contains
    # the note on and note off events.
    note_track_idx, note_track = get_note_track(mid)
    ...
    for i, msg in enumerate(note_track):
        if (msg.type == 'note_on' and msg.velocity > 0):
            msg.velocity=quantize_velocity
            
    new_track = note_track
    if new_track == None:
        return None
    quantized_mid.tracks[note_track_idx] = new_track
    return quantized_mid
...
input_dir = '../TPD database/classical'
print('from {0}'.format(input_dir))
ls = sorted(glob.glob(input_dir+'/*.mid'))

for file in ls:
    midi_data = mido.MidiFile(file)
#    midi_data = mido.MidiFile('../TPD database/classical/DANILO02.mid')
    quantized_mid=quantize(midi_data, 4)   
    quantized_mid=velocity_quantize(quantized_mid, 60)
    save_file_name=file[0:15]+'/quantized'+file[15:]
    quantized_mid.save(save_file_name)
