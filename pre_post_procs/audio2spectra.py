...
import glob
import pretty_midi
from utils import mkdir, read_via_scipy, get_spectrogram, get_cepstrogram, get_diff_spectrogram, get_spectral_envelope, plot_figure, ReLU, read_via_pretty
...
config = {
    ...
    'audio_samples_frame_size': 220500, # 10sec * sr
    'audio_samples_hop_length': 220500,
    'output_hei': 256,
    'output_wid': 862, # num_output_frames = 1 + (220500/hop_length256)   
...
def audio2npys(input_file, config):
    ...
    song_name = input_file.split('/')[-1][7:-4]
    print('!song_name = {}'.format(song_name))
    
    y, sr = read_via_pretty(input_file)
...
'''
inpath = './midi_wav_dataset/quantized/'
instrument = 'Chopin'
prefix = 'npy_dataset/_10sec_quantized' # naming follows the settings in config
...
for file in ls:
    ...
    # piano_roll = midi_data.get_piano_roll(pedal_threshold=None)
    # i =i+1


    # midi_data = pretty_midi.PrettyMIDI(file)
    # wave_y = midi_data.fluidsynth(22050,)
    # librosa.output.write_wav(wav_name, wave_y, 22050)


    # 转换mid to wav
    # wav_name = file[:-4] +'.wav'
    # s=FluidSynth('default_sound_font.sf2')
    # s.midi_to_audio(file, wav_name)
    # print('write to:' + wav_name)
